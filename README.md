...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-logging-type.git --from 0.1.0-preview
```

Then add `GoogleCloudLoggingType` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudLoggingType <target-name> --package swift-google-logging-type
```
